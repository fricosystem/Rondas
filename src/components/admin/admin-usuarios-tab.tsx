...
// ── Páginas exibidas no seletor de permissões (apenas Ronda) ─────────────────
// As demais permissões continuam sendo respeitadas pelo sistema, apenas não
// aparecem para seleção manual nesta tela.
const PAGINAS_VISIVEIS: PageType[] = ['ronda'];
              ...
              <div className="flex gap-1">
                <Button
                  ...
                  onClick={() => setFormData((prev) => {
                    if (!prev) return prev;
                    // Mantém permissões ocultas e adiciona todas as visíveis
                    const ocultas = prev.permissoes.filter((p) => !PAGINAS_VISIVEIS.includes(p));
                    return { ...prev, permissoes: [...ocultas, ...PAGINAS_VISIVEIS] };
                  })}
                >
                ...
                <Button
                  ...
                  onClick={() => setFormData((prev) => {
                    if (!prev) return prev;
                    // Remove apenas as permissões visíveis, mantendo as ocultas
                    return { ...prev, permissoes: prev.permissoes.filter((p) => !PAGINAS_VISIVEIS.includes(p)) };
                  })}
                >
