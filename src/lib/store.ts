...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  // Aba ativa da área administrativa — inicia em "rondas"
  adminTab: 'rondas',
  ...
  login: async (email: string, password: string) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') localStorage.setItem('apex_porter_currentPage', 'ronda');
      set({ isAuthenticated: true, user, authLoading: false, currentPage: 'ronda' });
      // Start Firestore core subscriptions, then activate ronda features
      startSubscriptions();
      getFeaturesForPage('ronda').forEach(activateFeature);
      return true;
    } catch (err: any) {
  ...
  setAuthFromFirebase: (firebaseUser: FirebaseUser, firestoreData?: FirestoreUser | null) => {
    ...
    const targetPage = get().currentPage === 'login' ? 'ronda' : get().currentPage;
