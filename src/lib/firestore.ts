  ...
  if (typeof obj === 'object') {
    ...
    for (const [key, value] of Object.entries(obj)) {
      if (value !== undefined) { // Ignora campos undefined para não enviar para Firestore
        // Chaves de referência (ex.: postoId, cargoId, usuarioId) guardam IDs de
        // documentos do Firestore e NUNCA podem ser convertidas para maiúsculas,
        // caso contrário o vínculo com o documento original é quebrado.
        if (DONT_UPPERCASE_KEYS.has(key) || key.endsWith('Id')) {
          newObj[key] = value;
        } else {
